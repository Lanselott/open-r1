...
def get_log_scaled_reward(
    ...
    def log_scaled_reward(completions, solution, **kwargs):
        """Reward function that scales based on completion length using a log schedule.
        ...
        for content, sol in zip(contents, solution):
            ...
            # Apply log scaling based on length
            progress = gen_len / max_len
            log = math.log(progress + 0.2, 5)
            ...
            reward = min_value + 0.5 * (max_value - min_value) * (1.0 - log)
            rewards.append(float(reward))
...
def get_reward_funcs(tokenizer, script_args) -> list[Callable]:
    REWARD_FUNCS_REGISTRY = {
        ...
        "log": get_log_scaled_reward(
            ...
            max_value_wrong=script_args.log_max_value_wrong,
            min_value_correct=script_args.log_min_value_correct,
            max_value_correct=script_args.log_max_value_correct,
            max_len=script_args.log_max_len,
        ),
